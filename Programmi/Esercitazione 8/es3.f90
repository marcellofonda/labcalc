...
real(kind=kr),dimension(nbody) :: mass=[1.989E30_kr, 5.90E24_kr, 1898.6E25_kr]
...
real(kind=kr),dimension(3,nbody)	:: pos,vel,f
 real(kind=kr)						:: dt,epot,ekin,vterra,vgiove,vcentro
 integer :: nstep,it
 ...
 pos(:,3)=[816.62e9_kr,0._kr,0._kr]
 
 vterra=29.29e3_kr	
 vgiove=12.44e3_kr
 
 vcentro=(vterra*massa(2)+vgiove*massa(3))/sum(massa)
 
 vel=0.												! supponiamo velocità inizialmente nulla per il Sole. Questo genererà                         
 vel(2,1)=-vcentro
 vel(2,2)=vterra-vcentro										! dei moti curiosi per il Sole stesso. Curiosi ma corretti.
 vel(2,3)=vgiove-vcentro
 ...
 do it = 1,nstep
    ...
    if (it<366 .or. it>12770)  write(unit=1,fmt=*)it,it*dt,pos,vel
    ekin = 0.5 * SUM(spread(massa,1,3) * vel**2)
